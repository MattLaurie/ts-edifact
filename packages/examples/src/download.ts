import {EdifactMessageSpecification, MessageStructureParser, UNECEMessageStructureParser} from "@ts-edifact/legacy";
import {Suffix} from "@ts-edifact/core";
import fs from "fs";
...
export function downloadMessageTypes(version: string, messageTypes: string[], location: string): void {
    for (const messageType of messageTypes) {
        const structParser: MessageStructureParser = new UNECEMessageStructureParser(version, messageType);
        structParser.loadTypeSpec()
            .then((result: EdifactMessageSpecification) => {
                persist(result, location);
            }).then(() => {
            console.log(`Stored definition for type ${messageType} of version ${version} at location ${location}`);
        }).catch((error: Error) => {
            console.warn(`Could not load Message structure and segment/element definitions for message type ${messageType} of version ${version}. Reason: ${error.message}`);
        });
    }
}

const ICS_MESSAGE_TYPES: string[] = [
    'CUSCAR',
    'CUSDEC',
    'CUSPED',
    'CUSREP',
...
downloadMessageTypes('D99B', ICS_MESSAGE_TYPES, outputDir)
